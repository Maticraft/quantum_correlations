...
for i in range(0, len(data_list) - 1):
    ...
    axs[i, 0].plot(data1[:, 3], data1[:, 2], 'o-', markersize = 2, markevery=2, linewidth = 1, label='Trace reconstruction of zero-discord states')
    axs[i, 0].plot(data1[:, 6], data1[:, 5], '-', markersize = 4, linewidth =1,  label='Trace reconstruction of separable states')
    axs[i, 0].plot(data2[:, 3], data2[:, 2], 'o--', markersize = 2, markevery=2, linewidth = 1, label='Separator reconstruction of zero-discord states')
    axs[i, 0].plot(data2[:, 6], data2[:, 5], '--', markersize = 4, linewidth =1,  label='Separator reconstruction of separable states')
    ...
    axs[i, 1].plot(data1[:, 0], data1[:, 7], 'o-', markersize = 2, linewidth = 1, markevery=5, label='Trace reconstruction of zero-discord states')
    axs[i, 1].plot(data1[:, 0], data1[:, 8], '-', markersize = 4, linewidth =1,  label='Trace reconstruction of separable states')
    axs[i, 1].plot(data2[:, 0], data2[:, 7], 'o--', markersize = 2, linewidth = 1, markevery=5, label='Separator reconstruction of zero-discord states')
    axs[i, 1].plot(data2[:, 0], data2[:, 8], '--', markersize = 4, linewidth =1,  label='Separator reconstruction of separable states')
...
plt.savefig('./plots/discord_arch_metrics.png', bbox_extra_artists=[lgd] + anns, bbox_inches='tight', format='png')
plt.close()
