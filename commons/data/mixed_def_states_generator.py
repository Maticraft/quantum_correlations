...
class MixedDefStatesGenerator():
    ...
    def generate_matrices(self, qubits_num, examples, save_data_dir = None, specified_method = None, start_index = 0, encoded = True, label_potent_ppt = False, with_permutations = False, format = 'npy', separator_loss_range = None):
        ...
        for i in range(self.num_examples):
            ...
            if save_data_dir:
                save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", ro, f"mixed from pure {m}", "unknown", save_data_dir, ppt = ppt_flag, separate_bipart= encoded, format=format, separator_loss_range=separator_loss_range)
            else:
    ...
    @generate_with_assertion(1000)
    def generate_multi_mixed_matrices(self, qubits_num, examples, num_pure_states, save_data_dir = None, specified_method = None, base_size = None, start_index = 0, encoded = True, label_potent_ppt = False, zero_neg = 'incl', fully_entangled = False, max_num_ps = None, discord = False, mixing_mode = 'outer', with_permutations = False, format = 'npy', separator_loss_range = None):
        self._initialize_generator(examples, qubits_num, with_permutations)
        ...
        for i in range(len(mixed_dms)):
            ...
            if save_data_dir:
                if specified_method == 'simple_non_product_zero_discord' or specified_method == 'random_non_product_zero_discord':
                    save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", ro, "random mixed from pure", "{}".format(nps), save_data_dir, ppt= label_potent_ppt, separate_bipart=encoded, zero_neg= zero_neg, not_ent_qbits=not_ent_qbits_i, discord = discord, trace_reconstruction=True, format=format, separator_loss_range=separator_loss_range)
                else:
                    save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", ro, "random mixed from pure", "{}".format(nps), save_data_dir, ppt= label_potent_ppt, separate_bipart=encoded, zero_neg= zero_neg, not_ent_qbits=not_ent_qbits_i, discord = discord, format=format, separator_loss_range=separator_loss_range)
            else:
                return_matrices.append(ro)
        if not save_data_dir:
            return return_matrices
