...
class PureStatesGenerator:
    ...
    def generate_circuit_matrices(self, qubits_num, examples, save_data_dir = None, random_gates = 2, specified_method = None, start_index = 0, encoded = True, fully_entangled = False, return_matrices = False, discord = False, noise = False, with_permutations = False, format = 'npy', separator_loss_range = None):
        # possible methods:
        ...
        if save_data_dir != None:
            for i in range(len(dens_matrices)):
                ...
                save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", ro, self.methods[m].__name__, entangled_qbits, save_data_dir, separate_bipart=encoded, discord=discord, format=format, separator_loss_range=separator_loss_range)
            if return_matrices:
    ...
    @generate_with_assertion(1000)
    def generate_random_haar_matrices(self, qubits_num, examples, save_data_dir = None, start_index = 0, encoded = True, return_matrices = False, discord = False, with_permutations = False, format = 'npy', separator_loss_range = None):
        self._initialize_generator(examples, qubits_num, with_permutations)
        ...
        if save_data_dir == None:
            ...
            for i in range(len(matrices)):
                save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + self.num_permutations*i}", matrices[i], f"random_vector", "unknown", save_data_dir, separate_bipart=encoded, discord=discord, format=format, separator_loss_range=separator_loss_range)
