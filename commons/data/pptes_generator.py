...
class PPTESGenerator():
    ...
    def generate_3qbits_pptes(self, qubits_num, examples, save_data_dir, entanglement_class = "Horodecki", randomization = False, start_index = 0, encoded = True, ppt = True, discord = False, with_permutations = False):
        ...
        for i in range(self.num_examples):
            ...
            if save_data_dir != None:
                save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", dm, f"{entanglement_class}_ppt_entangled_state", "unknown", save_data_dir, ppt=ppt, separate_bipart=encoded, discord=discord)
            else:
                matrices.append(dm)
    ...
    @generate_with_assertion()
    def generate_extended_3q_pptes(self, qubits_num, examples, save_data_dir, entanglement_class = "Horodecki", randomization = False, start_index = 0, encoded = True, ppt = True, discord = False, with_permutations = False):
        ...
        for i, dm in enumerate(dms_3q):
            ...
            save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + i}", ext_dm, f"extended_{entanglement_class}_pptes", f"3/{self.num_qubits}", save_data_dir, ppt = ppt, separate_bipart=encoded, not_ent_qbits=not_ent_qbits, discord = discord)


    # Generalization of Horodecki 2 x 4 PPTES for 2 x d dimension, i.e. this method applicates for arbitrary number of qubits
    @generate_with_assertion()
    def generate_2xd_pptes(self, qubits_num, examples, save_data_dir, randomization = False, start_index = 0, encoded = True, discord = False, with_permutations = False):
        self._initialize_generator(examples, qubits_num, with_permutations)
        dim = 2**self.num_qubits
        d = dim // 2
        ...
        for ex in range(self.num_examples):
            ...
            save_dens_matrix_with_labels(self.num_qubits, f"dens{start_index + ex}", dm, f"2xd_pptes", "unknown", save_data_dir, ppt=True, separate_bipart=encoded, discord = discord)
